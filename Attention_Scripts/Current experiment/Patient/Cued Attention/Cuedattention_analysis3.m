...
session3=uigetfile('Cued_attention3*.mat');
load (session3);
...
matfilename = sprintf('%s_CuedAttention3', ParticipantID);
save(matfilename)
...
Valid1 =(length(find(Matrix3(:,2)>=0.001&Matrix3(:,3)>=0.001&Matrix3(:,6)<0.001))/nrtrials)*100; %percentage valid trials
AccurateAll1 =(length(find(Matrix3(:,3)>=0.001))/nrtrials)*100; %percentage invalid trials

%% Matrix with valid values (and eyemax < 1)

Matrix3=Matrix3(find(Matrix3(:,3)>=0.001&Matrix3(:,6)<0.001),:);

...
session1=uigetfile('Localiser_prac*.mat');
load (session1);
...
matfilename = sprintf('%s_Localiserprac', ParticipantID);
save(matfilename)
...
Valid1 =(length(find(Matrix1(:,3)>=0.001&Matrix1(:,6)<0.001))/nrtrials)*100; %percentage valid trials
AccurateAll1 =(length(find(Matrix1(:,2)>=0.001))/nrtrials)*100; %percentage invalid trials

%% Matrix with valid values

Matrix1=Matrix1(find(Matrix1(:,3)>=0.001&Matrix1(:,8)<0.001),:);

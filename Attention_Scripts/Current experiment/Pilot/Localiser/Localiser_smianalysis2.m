...
matfilename = sprintf('%s_Localiser2', ParticipantID);
save(matfilename)

%% Get eye data files
M2 = [];
M2 = Atten_readEyeData('iViewXSDK_Matlab_Data_Localiser2 Samples.txt');
...
Matrix2(find(Matrix2(:,6)>=eyemax),6)=NaN;
Matrix2(find(Matrix2(:,7)>=eyemax),7)=NaN;% What is a suitable degree to filter out of this trial?

Valid2 =(length(find(Matrix2(:,3)>=0.001&Matrix2(:,6)>=0.001&Matrix2(:,7)>=0.001))/nrtrials)*100; %percentage valid trials
AccurateAll2 =(length(find(Matrix2(:,2)>=0.001))/nrtrials)*100; %percentage invalid trials
Valid_eye2 = (length(find(Matrix2(:,6)>=0.001&Matrix2(:,7)>=0.001))/nrtrials)*100; %valid trials for eye movements only

%% Matrix with valid values

Matrix2=Matrix2(find(Matrix2(:,3)>=0.001&Matrix2(:,6)>=0.001&Matrix2(:,7)>=0.001),:);

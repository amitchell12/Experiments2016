...
M.type = 'mat'; 
M.columns = { 'time' 'type' 'trial' 'lx' 'ly' 'rx' 'ry' 'fix' 'cue' 'target' 'trialNo' 'trialcounter'}; %columns of your matrix
M.matrix = [];
...
while ischar(tline)
%disp(tline)
    if regexp(tline, '^\d+\s+MSG\s+\d+\s+\# Message: FIXON') %all SMI messages will look something like this
        fix = 1;
        fixationTimestampSMI = 1; 
        trialNo = trialNo+1; 
    elseif regexp(tline, '^\d+\s+MSG\s+\d+\s+\# Message: FIXOFF')
        fix = 0;
    elseif regexp(tline, '^\d+\s+MSG\s+\d+\s+\# Message: CUEON')
        Cue= 1;
    elseif regexp(tline, '^\d+\s+MSG\s+\d+\s+\# Message: CUEOFF')
        Cue = 0;
    elseif regexp(tline, '^\d+\s+MSG\s+\d+\s+\# Message: TARGETON')
        Target = 1;
        trialcounter = 1;
        %saccadeTargetOn = 1; 
    elseif regexp(tline, '^\d+\s+MSG\s+\d+\s+\# Message: TARGETOFF')
        Target = 0;
    else
        d = regexp(tline, '^(\d+)\s+\w+\s+(\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+', 'tokens');
        if ~isempty(d)
            values = str2double(d{1});
            M.matrix = [ M.matrix ; [values fix Cue Target trialNo trialcounter]];
            k = k + 1;
            trialcounter = 0;
        end
    end

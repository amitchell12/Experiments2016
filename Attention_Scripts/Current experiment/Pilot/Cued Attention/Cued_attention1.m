...
%% Start the trials
<<<<<<< HEAD
        j = 0; %for no cue trials
        for i=1:length(targetss)
            %% Unsure if this works.. test
            if j == 1; %if no cue trial just presented, go back in the index so target locs all played
                i(i) = i-1;
                j = 0;
            end
...
%% Cue presentation
<<<<<<< HEAD
            if Cue(i) == 0; %no cue present, just target therefore do nothing
                j = j+1; %counting number of no cue trials
            else %Cue == 1; cue present
                cue = 'S';

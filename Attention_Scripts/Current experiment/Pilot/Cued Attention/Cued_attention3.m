...
[Data,Text] = xlsread('TrialCounter_cue3.xlsx');
...
datenow = clock;
matfilename = sprintf ('Cued_attention3%d%d%d%d%d.mat', datenow (1), datenow(2), datenow(3), datenow(4), datenow(5)); %name of the output file
...
%% END, CLOSE AND SAY THANKS
    ...
    if smi       
        ...
        ovr = int32(1);
        filename = formatString(256, int8(['D:\iViewXSDK_Matlab_Data_3' user '.idf'])); %This shows the pathway for saving - D:\ drive was the one that worked.  
...
Valid3=(length(find(Matrix3(:,3)>=0.001))/nrtrials)*100; %percentage of trials that are valid
Accurate3=(length(find(Matrix3(:,2)>=0.001))/nrtrials)*100; %percentage of trials that are accurate

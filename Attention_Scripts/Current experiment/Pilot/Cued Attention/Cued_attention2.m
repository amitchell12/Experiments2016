...
[Data,Text] = xlsread('TrialCounter_cue2.xlsx');
...
datenow = clock;
matfilename = sprintf ('Cued_attention2%d%d%d%d%d.mat', datenow (1), datenow(2), datenow(3), datenow(4), datenow(5)); %name of the output file
...
%     Screen('Preference', 'VisualDebugLevel', 0 );
    ...
    tic
        ...
        %sizecue=sizecuedeg*vadx; %size of cue in pixels, from degrees (diameter)
...
%% END, CLOSE AND SAY THANKS
    ...
    if smi       
        %save iViewX file
        user = formatString(64, int8('CuedAttention')); %Put your own file name in quotes here
        description = formatString(64, int8('Description1')); %Add some description if you like
        ovr = int32(1);
        filename = formatString(256, int8(['D:\iViewXSDK_Matlab_Data_2' user '.idf'])); %This shows the pathway for saving - D:\ drive was the one that worked.  
...
Valid2=(length(find(Matrix2(:,3)>=0.001))/nrtrials)*100; %percentage of trials that are valid
Accurate2=(length(find(Matrix2(:,2)>=0.001))/nrtrials)*100; %percentage of trials that are accurate

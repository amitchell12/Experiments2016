...
%% Frame rectangle version of the Posner task
% Presents a target (A or H) on the left or the right that participants
% have to identify vocally
% Some conditions (160/240) a cue is present (either on the left or the
% right) and this is followed by a target (valid/invalid), the task is the
% same as before
% Cue is a framed rectangle that is flashed up before the target on either the left or right 
% Frames on the left and right are displayed after cue, before target and
% after target - so participant knows to respond
% 240 trials in total, 6 conditions (40 trials per condition)
...
rectduration=0.06; %length of checker, seconds
...
sizecuedeg=1.5; %deg visual angle size of the cue rectangle

%% Trial variables

% read trial variables from an xls file
[Data,Text] = xlsread('AttentionChecker.xlsx');
...
% Conditions:
% 0,1: no cue left; 0,2: no cue right; 1,1: left cue left target; 1,2: left
% cue right target; 2,1: right cue left target; 2,2: right cue right target

nrtrials = length(Cue); %trials, total nr
%nrtrials = 20; %for tesing

%% correct response keys
leftKey = KbName('-'); %this is a number that identifies the key; letter A
rightKey = KbName('+'); %letter H
% bothKeysAS = KbName('2'); %letter A and S
% bothKeysHS = KbName('5'); %letter H and S
%% OUTPUT
...
datenow = clock;
matfilename = sprintf ('AttFrame%d%d%d%d%d.mat', datenow (1), datenow(2), datenow(3), datenow(4), datenow(5)); %name of the output file
...
%     Screen('Preference', 'VisualDebugLevel', 0 );
    ...
    penwidth = 4; %width of frame for rectangle
...
%% FRAMERECT PRESENTATION
            if Cue(i) == 1 % left frame rect cue
                ...
                Screen('Flip', window);
                timecueends(i) = GetSecs;
                if smi
                    calllib('iViewXAPI', 'iV_SendImageMessage', formatString(256, int8('FRAMEOFF'))); % send iViewX 'cueoff' message
                end
                cuetime= -rectduration + recttimemin + (recttimemax-recttimemin).*rand(1); %random nr in the interval 0.1-0.2 
                WaitSecs(cuetime); %wait for total time of cue
            elseif Cue(i) == 2 % right frame rect cue
                ...
                WaitSecs(rectduration); %wait cue duration
                cuetime= -rectduration + recttimemin + (recttimemax-recttimemin).*rand(1); %random nr in the interval 0.1-0.2 
                WaitSecs(cuetime); %wait for total time of cue
            end
...
%% TARGET PRESENTATION
                   ...
                   if TargetSide(i) == 1; %if target is presented on the left
                        ...
                        Screen('DrawText', window, target, DisplayXSize/2-xcue, DisplayYSize/2-heightt/2, foregroundColor); %draw target
                        Screen('Flip', window); %show my target                   
                        timetargetstart(i)=GetSecs; %get time of target presentation

                   else %target presented on the right (==2)
                        widtht=RectWidth(Screen ('TextBounds', window, target)); %width of the target
                        heightt=RectHeight(Screen ('TextBounds', window, target)); %height of the target
                        Screen('DrawText', window, text, DisplayXSize/2 - width/2, DisplayYSize/2-height/2, foregroundColor); %draw fixation cross
                        Screen('DrawText', window, target, DisplayXSize/2+xcue, DisplayYSize/2-heightt/2, foregroundColor); %draw target
                    ...
                    %flip to two rects on screen, no fix cross

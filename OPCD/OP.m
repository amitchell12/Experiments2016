...
expName = 'OP_s1'; %All files from this experiment will be saved as this
...
%% IMPORT TRIAL MATRIX 
if practice == 0 
    [Data,Text] = xlsread('TrialMatrix'); 
else 
    [Data,Text] = xlsread('TrialMatrixPractice'); %Practice;
end 
 ...
 %% Experiment starts
   ...
   for i=1:nrtrials
       ...
       if Cond(i) == 1 %push trial
           if eet
              Eyelink('Message', 'PUSHON'); %send eet message that eye is being pushed
           end
           Snd('Play', beep);
           WaitSecs(3);
           Snd('Quiet');
           if eet
              Eyelink('Message', 'PUSHEND'); %send eet message push trial has stopped
           end
       else %Cond(i) == 0, control (no push) trial
           if eet
               Eyelink('Message', 'CONTROLON'); %send eet message that it is a control trial
           end
           Snd('Play', cbeep);
           WaitSecs(3)
           Snd('Quiet');
           if eet
               Eyelink('Message', 'CONTROLEND'); %send eet message that control trial stopped
           end           
       end
       if eet
          Eyelink('Message', 'SOUNDEND'); %send eet message that sound has stopped
       end
       WaitSecs(1); %wait 1s

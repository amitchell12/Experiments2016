...
load (filename);
matfilename = sprintf('CD_%s', ppt); %name of mat.file
...
ms_postsound = 3500; %ms to analyse after sound
...
fours = 1001; %the four second mark (row) in the time vector
...
EyeCD = [];
EyeCD(:,1) = time; %time in s
EyeCD(:,2) = soundon; %sound on marker
EyeCD(:,3) = push;
EyeCD(:,4) = LXAmp_deg; %left eye x amplitudes (deg)
EyeCD(:,5) = LYAmp_deg; %left eye y amplitudes (deg)
EyeCD(:,6) = RXAmp_deg; %right eye x amplitudes (deg)
EyeCD(:,7) = RYAmp_deg; %right eye y amplitudes (deg)
EyeCD(:,8) = XAmp_deg; %both eye x amplitudes (deg)
EyeCD(:,9) = YAmp_deg; %both eye y amplitudes (deg)
EyeCD(:,10) = trialcounter; %to count the number of trials (deg)
...
%CD condition - use when analysing CD data (in the dark)
% Left X amplitude
LXampCD = figure(1);
...
I= find(EyeCD(:,10));%finds the nonzero elements, when sound was presented
I= I(2:end-1);
...
for i=1:length(I); %indexing
    ...
    %on
    lxCD = LXAmp_deg(I(i)-presound:I(i) + postsound); % values for left eye x deg
    rxCD = RXAmp_deg(I(i)-presound:I(i) + postsound); % values for right eye x deg
    lyCD = LYAmp_deg(I(i)-presound:I(i) + postsound); % values for left eye y deg
    ryCD = RYAmp_deg(I(i)-presound:I(i) + postsound); % values for right eye y deg
    
    tlxCD = (lxCD) - MLXbs(i); %translated values for left eye x deg
    trxCD = (rxCD) - MRXbs(i); %translated values for right eye x deg
    tlyCD = (lyCD) - MLYbs(i); %translated values for left eye y deg
    tryCD = (ryCD) - MRYbs(i); %translated values for right eye y deg

    %% PLOTS
    xsoundmin = 0;
    xsoundmax = 4000; %time window along x-axis in ms
...
for i = 1:length(LXMat);
    av_lxCD(i) = mean(LXMat(i, :));
    av_rxCD(i) = mean(RXMat(i, :));
end
% Do the same for Y values
for i = 1:length(LYMat);
    av_lyCD(i) = mean(LYMat(i, :));
    av_ryCD(i) = mean(RYMat(i, :));
end
...
figure(11)
plot(soundwindow, av_lxCD);
hold on
plot(soundwindow, av_rxCD);
hold on
axis([xsoundmin xsoundmax ymin ymax]);
scatter(soundwindow, sound, 'k*');
xlabel('Time (ms)'); ylabel('Saccade amplitude (deg)');
...
figure(12)
plot(soundwindow, av_lyCD);
hold on
plot(soundwindow, av_ryCD);
hold on
scatter(soundwindow, sound, 'k*');
axis([xsoundmin xsoundmax ymin ymax]);
xlabel('Time (ms)'); ylabel('Saccade amplitude (deg)');

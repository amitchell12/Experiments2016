...
% $Id: brede_read_edf.m,v 1.2 2010/10/12 09:24:03 fn Exp $
  ...
  M.type = 'mat'; 
  M.columns = { 'time' 'lx' 'ly' 'lp' 'rx' 'ry' 'rp' 'saccade' 'blink' 'soundon' 'block' 'trialcounter'};
  M.matrix = [];
  ...
  while ischar(tline)
    ...
    elseif soundon & regexp(tline, '^MSG\s+\d+\s+SOUNDEND') 
      ...
      d = regexp(tline, '^(\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)\s+(\d+\.\d+)', 'tokens');
      if ~isempty(d)
	values = str2double(d{1});
	M.matrix = [ M.matrix ; [ values saccade blink soundon block trialcounter] ];
	k = k + 1;
    trialcounter = 0;
      end
    end
  ...
  fclose(fid);
     ...
     % Samples that are not in saccades and not in eyeblink and is within a block
      I = find(~any(M.matrix(:,8:9),2) & M.matrix(:,8)); %this returns the indices of non-saccade, non-blinks elements AND that are within the INBLOCK
      X = M.matrix(I,:);
      ...
      for id = blockids
        A.matrix(id,:) = [ id mean(X(X(:,11)==id,2)) mean(X(X(:,12)==id,3)) ];
      end
